...
class _FakeActivityStatsNotifier extends ActivityStatsNotifier {
  _FakeActivityStatsNotifier(this._stats);

  final ActivityStats _stats;

  @override
  Future<ActivityStats> build() async => _stats;
}
  ...
  final container = ProviderContainer(
    overrides: [
      ...
      activityStatsNotifierProvider.overrideWith(
        () => _FakeActivityStatsNotifier(_stats),
      ),
      activityCountProvider(null).overrideWith((ref) async => count),
    ],
  );
...
void main() {
  group('ActivitiesSummaryTabWidget', () {
    ...
    testWidgets('fetch failure → shows the empty state (no error widget)', (
      tester,
    ) async {
      final container = await _buildContainer(count: 0);
      addTearDown(container.dispose);
      ...
      await tester.pumpWidget(
        _harness(
          ...
          ActivitiesSummaryTabWidget(
            filter: _filter,
            onFetch: ({required filter, required pagination}) async {
              return const Failure<List<Activity>>(
                UnknownFailure('boom'),
              );
            },
          ),
